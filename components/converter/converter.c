...
int64_t  converter_bytes_to_int64(uint8_t * bytes) {
  for (int8_t i = 0; i < sizeof(int64_t); i++) {
    bytes_and_int64.bytes[(sizeof(int64_t) - 1 - i) * IS_LSB + (1 - IS_LSB)* i] = bytes[i];
  }
...
int32_t converter_bytes_to_int32(uint8_t * bytes) {
  for (uint8_t i = 0; i < sizeof(int32_t); i++) {
    bytes_and_int32.bytes[(sizeof(int32_t) - 1 - i) * IS_LSB + (1 - IS_LSB)* i] = bytes[i];
  }
...
void converter_int32_to_bytes(int32_t val, uint8_t * bytes) {
  ...
  for(int8_t i = 0; i < sizeof(int32_t); i++) {
    bytes[i] = int32_and_bytes.bytes[(sizeof(int32_t) - 1 - i) * IS_LSB + (1 - IS_LSB)* i];
  }
}
